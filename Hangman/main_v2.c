...
//returning difficulty level upon user command
char choose_level(char words[ROW][COL], char level)
{
    if (level == 'r')
    {
        char regular_words[ROW][COL] = {"vampire", "potato", "banana", "unicorn", "logitech", "pizza", "yellow", "north", "salad", "jupiter"};
        for (int i = 0; i < ROW; i++)
        {
            for (int j = 0; j < COL; j++)
            {
                words[i][j] = regular_words[i][j];
            }
        }
    }
    else if (level == 's')
    {
        char simple_words[ROW][COL] = {"apple", "goat", "table", "south", "bus", "fan", "cat", "mouse", "rice", "tree"};
        for (int i = 0; i < ROW; i++)
        {
            for (int j = 0; j < COL; j++)
            {
                words[i][j] = simple_words[i][j];
            }
        }
    }
...
replay:;
    char words[ROW][COL];
    int difficulty;
    printf("Which mode do you want to play?\nType 's' for simple mode or 'r' for regular mode:");
    scanf("%c", &difficulty);
    choose_level(words, difficulty);
    clear_screen();
    ...
    if (replay_command == 1)
    {
        // calling the game for going to line 215 for replay
        goto replay;
    }
    else
